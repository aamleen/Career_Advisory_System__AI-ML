...
breadth_FS(_Dst,[],_).
breadth_FS(Dst, Travelled, Route):-
        Travelled = [Current|_],
        Current \== Dst,
        findall(NextCity,
        ...
        breadth_FS(Dst, NewTravelled, [Current|Route]).

breadth_FS(Dst, [Dst|_], Reverse_Route):-
        writeln('--------------------'),
        ...
        actual_dist(Route,Dst,TotalDist),
        write("The total Distance is: "), writeln(TotalDist).
